    ...
    for _ in range(max_ls_iter):
        ...
        if f_new > f_k + c1 * alpha * dir_deriv:
            ...
            if abs(dir_deriv_new) > c2 * abs(dir_deriv):
                alpha_min = alpha
                alpha = alpha * 2 if alpha_max == np.inf else (alpha_min + alpha_max) / 2
            else:
                return alpha
...
def optimize_gradient_descent(expr, vars_sym, x0, tol, max_iter, alpha_init, c1, c2):
    ...
    while error > tol and iteration < max_iter:
        p = -g_val
        # Usar el alpha inicial configurado en la primera iteración
        if iteration == 0:
            alpha = alpha_init
        else:
...
# [El resto de tus funciones: optimize_conjugate_gradient, optimize_newton, 
# init_session_state, show_sidebar, show_login_page, show_config_page, 
# show_results_page y main deben seguir aquí abajo, manteniendo la misma indentación]

# Asegúrate de mantener todo el bloque de tu script con este formato de indentación.
